...
const App: React.FC = () => {
  ...
  const handleCapture = useCallback((enclosedIds: string[], loc: Point) => {
    ...
    // 3) Persistir células no servidor (batch)
    ...
    // 4) Limpar rastro e continuar do ponto de fechamento
  ...
  // Engine: detectar movimento e loops
  useEffect(() => {
  ...
  // Inicializador: puxar snapshot global (quando entra em HOME)
  useEffect(() => {
  ...
  // ✅ POLLING GLOBAL: atualiza cells + usuários ativos em tempo real
  useEffect(() => {
    if (!user) return;
    ...
    const poll = async () => {
      try {
        ...
        // silencioso pra não poluir console
      }
    };
  ...
  const stopActivity = () => {
    // Apenas troca para summary (finalização real acontece no "CONCLUIR MISSÃO")
        ...
        <div className="absolute inset-x-0 bottom-10 p-6 flex flex-col gap-4 pointer-events-none z-[1000]">
          <div className="bg-black/90 backdrop-blur-2xl p-4 rounded-3xl border border-white/10 flex justify-between items-center pointer-events-auto shadow-2xl">
            <div className="flex gap-4 items-center">
              <div
                className="w-12 h-12 rounded-2xl bg-blue-600/20 flex items-center justify-center font-black italic border border-blue-500/30 text-blue-500 text-xl shadow-inner"
              >
                {user.nickname[0]}
              </div>
              <div>
